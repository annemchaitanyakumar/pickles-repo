    ...
    <div className="min-h-screen bg-gradient-warm">
      ...
      <section className="pt-24 pb-16">
        <div className="max-w-[1400px] mx-auto">
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.4 }} className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {products.map((product, idx) => {
              ...
              return (
                <motion.div key={product.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3, delay: idx * 0.05 }} className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow duration-300 w-full" onMouseEnter={() => setIsHovered(prev => ({ ...prev, [product.id]: true }))} onMouseLeave={() => setIsHovered(prev => ({ ...prev, [product.id]: false }))}>
